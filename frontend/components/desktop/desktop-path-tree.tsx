...
/**
 * 桌面专属·路径学习资料树（独立副本，桌面尺度略大）。
 * 复用共享的 AGENT_ICONS/AGENT_MAP（图标与配置原子，非 web 布局）。
 */
export function DesktopPathTree({
  path,
  completed,
  onToggle,
  title = "学习路径",
  ...
  return (
    <section className="rounded-2xl border bg-card p-5">
      <div className="flex items-center gap-1.5">
        ...
        <h2 className="text-[15px] font-semibold">学习资料树</h2>
      ...
      <div className="mt-4">
        <div className="inline-flex items-center gap-1.5 rounded-md bg-danger px-2.5 py-1 text-[13px] font-semibold text-white">
            ...
            return (
              <li key={`${stage.day}-${i}`} className="relative">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="truncate text-[13px] font-medium">{stage.title}</span>
                </div>

                <ul className="ml-1 mt-2 space-y-1.5 border-l border-border/70 pl-3.5">
                  {stage.types.map((t, ti) => {
                    ...
                    return (
                      <li key={`${t}-${ti}`} className="flex items-center gap-2">
                        <Icon
                          className={cn("size-3.5 shrink-0", isDone && "text-success")}
                        ...
                        >
                          {typeName(t)}
                          {isDone && <span className="ml-1 text-[11px]">· 已完成</span>}
                        </span>
                        <button
                          type="button"
