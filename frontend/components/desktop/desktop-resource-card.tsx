...
/**
 * 桌面专属资源卡——独立副本，桌面可自由改样式（不碰 web 的 ResourceCard）。
 * 复用共享原子：AgentIconTile（图标）/ Badge / Skeleton。
 */

function ResourcePreview({ type, pending }: { type: ResourceType; pending: boolean }) {
  if (pending) {
    return (
  ...
  switch (type) {
    case "explainer":
      return (
        <div className="flex h-16 flex-col justify-center gap-1.5 rounded-lg bg-muted/60 px-3">
    ...
    case "mindmap":
      return (
        <div className="grid h-16 place-items-center rounded-lg bg-muted/60">
          <svg viewBox="0 0 190 44" className="h-12 w-44">
    ...
    case "quiz":
      return (
        <div className="flex h-16 flex-col justify-center gap-1.5 rounded-lg bg-warning/[0.07] px-3 text-[10px]">
          <div className="flex items-center gap-1.5">
    ...
    case "code":
      return (
        <div className="flex h-16 flex-col justify-center gap-1 rounded-lg bg-zinc-900 px-3 font-mono text-[9px] leading-snug">
    ...
    case "video":
      return (
        <div className="relative h-16 overflow-hidden rounded-lg bg-zinc-900">
          ...
          <div className="absolute inset-0 grid place-items-center">
            <span className="grid size-8 place-items-center rounded-full bg-danger">
    ...
    case "reading":
      return (
        <div className="flex h-16 flex-col justify-center gap-1.5 rounded-lg bg-success/[0.07] px-3">
    ...
    case "interactive":
      return (
        <div className="relative h-16 overflow-hidden rounded-lg bg-[#101a17]">
          {/* 用一组透视线框暗示「可旋转的三维模型」，纯 CSS，无需真的起 WebGL */}
          <div className="absolute inset-0 grid place-items-center">
            <span className="block size-9 rotate-12 border border-[#6fd0a1]/70" />
            <span className="absolute size-9 -rotate-12 border border-[#6fd0a1]/35" />
          </div>
    ...
    default:
      return <div className="h-16 rounded-lg bg-muted" />;
  }
}
...
function StatusBadge({ status }: { status: ResourceStatus }) {
  switch (status) {
    ...
    case "rejected":
      return (
        <Badge variant="outline" className="gap-1 border-danger/40 bg-danger/10 text-[10px] text-danger">
    ...
    case "ready":
      return (
        <Badge variant="outline" className="gap-1 border-success/40 bg-success/10 text-[10px] text-success">
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <span className="truncate text-sm font-semibold leading-tight">{item.title}</span>
            {item.version > 1 && (
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] text-muted-foreground">
        {item.meta.map((m, i) => (
      ...
      {clickable && (
        <div className="mt-2.5 flex items-center justify-end gap-0.5 border-t pt-2.5 text-xs font-medium text-primary">
          {item.type === "quiz" ? "开始答题" : "查看详情"}
